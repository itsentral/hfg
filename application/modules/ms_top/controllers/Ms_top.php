...
class Ms_top extends Admin_Controller
{
	...
	public function data_side()
	{
		...
		foreach ($query->result_array() as $row) {
			...
			$nestedData[] = "<div class='text-start'>{$name}</div>";

			// Nilai (angka -> end/right)
			$nilai = $row['data1'] ?? '';
			// kalau numeric, format biar rapi
			if (is_numeric(str_replace([','], [''], $nilai))) {
				$nilai = number_format((float)str_replace(',', '', $nilai));
			}
			$nestedData[] = "<div class='text-end'>{$nilai}</div>";
			...
			// kalau mau pakai akses, tinggal aktifkan lagi kondisinya
			// if ($Arr_Akses['update'] == '1') {
			$editBtn = "<button type='button' class='btn-icon btn-icon-edit edit' data-code='{$id}' title='Edit'>
                        <i class='ti ti-edit'></i>
                    </button>";
			// }

			// if ($Arr_Akses['delete'] == '1') {
			$delBtn  = "<button type='button' class='btn-icon btn-icon-delete delete' data-code='{$id}' title='Delete'>
                        <i class='ti ti-trash'></i>
                    </button>";
			// }

			$nestedData[] = "<div class='text-end'>{$editBtn} {$delBtn}</div>";
