...
<style>
    .swal2-container.swal2-center { z-index: 9999 !important; }
    .badge-status { font-size: 13px; padding: 5px 12px; border-radius: 4px; }
    ...
    .table-jurnal th { background: #f8f9fa; }
</style>
    ...
    $('#btnPostAccounting').on('click', function () {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.post('<?= base_url("gl_interface/post") ?>', { id: '<?= $header['id'] ?>' }, function (res) {
                    if (res.status == 1) {
                        Swal.fire({
                            icon: 'success',
                            title: 'Berhasil!',
                        ...
                        Swal.fire({ icon: 'error', title: 'Gagal', text: res.pesan });
