...
<script>
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            // Siapkan data gudang dari PHP ke JS Object sekali saja
            const listGudang = <?= json_encode($list_gudang); ?>;
            ...
            if (data && data.length > 0) {
                data.forEach((item, index) => {
                    ...
                    // Grouping berdasarkan Material
                    if (item.id_material !== currentMaterial) {
                        let qty_po = parseFloat(item.qty_po) || 0;
                        let qty_in = parseFloat(item.qty_in) || 0;
                        let qty_belum_kirim = qty_po - qty_in;

                        rowMaterial = `
                            <td class="text-center">${index + 1}</td>
                            <td><b>${item.nm_material || '-'}</b></td>
                            <td class="text-right">${qty_po.toLocaleString('id-ID')}</td>
                            <td class="text-center">Kg</td>
                            <td class="text-right">${qty_belum_kirim.toLocaleString('id-ID')}</td>
                        `;
                        currentMaterial = item.id_material;
                    } else {
                        // Gunakan baris kosong untuk material yang sama agar terlihat rapi (Grouping)
                    ...
                    <tr>
                        ${rowMaterial}
                        <td class="bg-light text-center">${item.no_coil}</td>
                        <td class="bg-light text-end">${parseFloat(item.ros_kotor).toLocaleString('id-ID')}</td>
                        <td class="bg-light text-end">${parseFloat(item.ros_bersih).toLocaleString('id-ID')}</td>
                ...
                html = '<tr><td colspan="11" class="text-center">Data tidak ditemukan atau ROS kosong.</td></tr>';
            }
            
            $('#list-item-coil').html(html);
            // Re-inisialisasi select2 untuk element baru
            $('.select-gudang-coil').select2({ width: '100%', dropdownAutoWidth: true });
        },
        error: function() {
            $('#list-item-coil').html('<tr><td colspan="11" class="text-center text-danger">Gagal mengambil data dari server.</td></tr>');
        }
    });
}
    ...
    const kdGudang = selectedOption.data('kd') || '';
    $(this).siblings('.kd-gudang-coil').val(kdGudang);
});
